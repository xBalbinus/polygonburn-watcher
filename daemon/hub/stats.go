...
import (
	...
	"github.com/maticnetwork/bor/tree/master/common/hexutil"
	"github.com/maticnetwork/bor/tree/master/core/types"
	"github.com/xBalbinus/polygonburn-watcher/daemon/sql"
)
...
func (s *Stats) initialize(
	...
	log.Infof("Initialize rpcClientHttp '%s'", borEndpointHTTP)

	s.rpcClient = &RPCClient{
		endpoint:   borEndpointHTTP,
...
func (s *Stats) initWaitForSyncingFalse() error {
	...
	for borSyncing {
		borSyncingRaw, err := s.rpcClient.CallContext(
			"2.0",
			"bor_syncing",
			"",
			true,
		...
		if err != nil {
			return fmt.Errorf("failed to get syncing status from bor: %v", err)
		}

		err = json.Unmarshal(borSyncingRaw, &borSyncing)
		if err != nil {
			err = json.Unmarshal(borSyncingRaw, &s.borSyncing)
			if err != nil {
				return fmt.Errorf("couldn't unmarshal bor_syncing response: %v", borSyncingRaw)
			}

			current, err := hexutil.DecodeUint64(s.borSyncing.CurrentBlock)
			if err != nil {
				return fmt.Errorf("couldn't decode bor_syncing CurrentBlock: %v", err)
			}

			highest, err := hexutil.DecodeUint64(s.borSyncing.HighestBlock)
			if err != nil {
				return fmt.Errorf("couldn't decode bor_syncing HighestBlock: %v", err)
			}

			log.Infof("init: bor is syncing: %d/%d", current, highest)
		}
		if borSyncing {
	...
	log.Infof("init: bor syncing finished")
	s.borSyncing = nil
...
func (s *Stats) updateLatestBlock() (uint64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("failed to fetch latest block number from bor: %v", err)
	}
...
func (s *Stats) getTotalsTimeDelta(startTime uint64, endTime uint64) (Totals, error) {
	...
	MATIC := big.NewInt(1_000_000_000_000_000_000)
	burned.Div(burned, MATIC)
	issuance.Div(issuance, MATIC)
	rewards.Div(rewards, MATIC)
	tips.Div(tips, MATIC)
...
func (s *Stats) updateBlockStats(blockNumber uint64, updateCache bool) (sql.BlockStats, []sql.BlockStatsPercentiles, error) {
	...
	if err != nil {
		return blockStats, blockStatsPercentiles, fmt.Errorf("error bor_getBlockByNumber: %v", err)
	}

	block := Block{}
	err = json.Unmarshal(rawResponse, &block)
	if err != nil {
		return blockStats, blockStatsPercentiles, fmt.Errorf("error bor_getBlockByNumber Unmarshal Block: %v", err)
	}

	header := types.Header{}
	err = json.Unmarshal(rawResponse, &header)
	if err != nil {
		return blockStats, blockStatsPercentiles, fmt.Errorf("error bor_getBlockByNumber Unmarshal Header: %v", err)
	}
	...
	for n, uncleHash := range block.Uncles {
		...
		raw, err := s.rpcClient.CallContext(
			"2.0",
			"bor_getUncleByBlockNumberAndIndex",
			strconv.Itoa(int(blockNumber)),
			updateCache,
			blockNumberHex,
		...
		if err != nil {
			return blockStats, blockStatsPercentiles, fmt.Errorf("error bor_getUncleByBlockNumberAndIndex: %v", err)
		}

		uncle := Block{}
		err = json.Unmarshal(raw, &uncle)
		if err != nil {
			return blockStats, blockStatsPercentiles, fmt.Errorf("error bor_getUncleByBlockNumberAndIndex Unmarshal uncle: %v", err)
		}
	...
	mMATIC := big.NewInt(1_000_000_000_000_000)
	blockReward.Div(&blockReward, mMATIC)
	blockTips.Div(blockTips, mMATIC)
	blockBurned.Div(blockBurned, mMATIC)

	duration := time.Since(start) / time.Millisecond
	log.Printf("block: %d, blockHex: %s, timestamp: %d, gas_target: %s, gas_used: %s, rewards: %s mMATIC, tips: %s mMATIC, baseFee: %s GWEI, burned: %s mMATIC, transactions: %s, type2: %s, ptime: %dms", blockNumber, blockNumberHex, header.Time, gasTarget.String(), gasUsed.String(), blockReward.String(), blockTips.String(), baseFee.String(), blockBurned.String(), transactionCount.String(), type2count.String(), duration)
