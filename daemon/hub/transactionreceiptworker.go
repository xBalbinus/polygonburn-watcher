...
func (h *TransactionReceiptWorker) processTransactionReceipt(rpcClient *RPCClient, param transactionReceiptJob) (*transactionReceiptResponse, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("error bor_getTransactionReceipt: %v", err)
	}

	receipt := TransactionReceipt{}
	err = json.Unmarshal(raw, &receipt)
	if err != nil {
		return nil, fmt.Errorf("error bor_getTransactionReceipt Unmarshal TransactionReceipt: %v", err)
	}
