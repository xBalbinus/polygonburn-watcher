...
import (
	...
	borRPC "github.com/maticnetwork/bor/tree/master/rpc"
	"github.com/gorilla/websocket"
	"github.com/xBalbinus/polygonburn-watcher/daemon/version"
...
func New(
	...
	s.initialize(borEndpointHTTP, dbPath, ropsten, workerCount)

	h.initializeWebSocketHandlers()
	err := h.initializeGrpcWebSocket(borEndpointWebsocket)
	if err != nil {
		return nil, err
	}
...
func (h *Hub) initializeWebSocketHandlers() {
	h.handlers = map[string]func(c *Client, message jsonrpcMessage) (json.RawMessage, error){
		...
		"bor_syncing":                       h.ethSyncing(),

		// proxy to geth

		// proxy to rpc
		"bor_subscribe":   h.borSubscribe(),
		"bor_unsubscribe": h.borUnsubscribe(),
	}
}
...
func (h *Hub) initializeGrpcWebSocket(gethEndpointWebsocket string) error {
	...
	sub, err := gethRPCClientWebsocket.BorSubscribe(context.Background(), headers, "newHeads")
	if err != nil {
	...
	go func() {
		<-error_chan
		log.Errorln("Reconnecting to Bor WS")
		h.initializeGrpcWebSocket(borEndpointWebsocket)
	}()
...
func (h *Hub) borSyncing() func(c *Client, message jsonrpcMessage) (json.RawMessage, error) {
...
func (h *Hub) borSubscribe() func(c *Client, message jsonrpcMessage) (json.RawMessage, error) {
	return func(c *Client, message jsonrpcMessage) (json.RawMessage, error) {
